...
namespace Screen1
{
    public delegate void SaveScreenDelegate(int x, int y, int width, int height, Bitmap bmp);
        ...
        private void FormScreenSave_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left && catchFinish)
            {
                ...
                if (catchRec.Width > 0 && catchRec.Height > 0)
                {
                    ...
                    // 将图片传递给事件
                    SaveScreenEvent(endPoint.X, endPoint.Y, catchedBmp.Width, catchedBmp.Height, catchedBmp);
                }
                catchFinish = false;
